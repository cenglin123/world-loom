...
### 人类作者
1. 填写 `00_世界观/核心设定.md` — 按物理层（世界法则 + 力量代价表 + 地理）→ 社会层（资源→势力→阶级）→ 张力层（最骄傲什么 / 可失效设定 / 故事入口）三层逐项填写
2. 填写 `00_世界观/外围设定.md` — 地理细节、次要门派等。可参考内置的以筠 #1 地图生成法和 #3 城镇选址评分法
3. 填写 `01_大纲/主线.md` — 主线一句话 + 矛盾来源 + 主线来源表（世界观最骄傲什么 → 怎么让它坏 → 主线方向）
4. 选定 `docs/writing-style.md` 中的视角、时态和文风倾向
5. 创建人物卡：`python scripts/check_tags.py wizard <角色名>` — 自动从模板创建，输出 15 个字段的待完善清单（带引导提示）
6. 开始写作——agent 会在提交前自动跑就绪自检 + 伏笔表校验 + 关系完整性 + 标签完整性 + 时空一致性
...
| 机制 | 检查项 |
|------|--------|
| **pre-commit 阻断** | AGENTS.md 同步 / 伏笔表格式 / 关系 JSON 完整性 / 标签五类完整性 / 章节时空一致性 + 就绪自检 |
