...
本仓分三层：**源码层**（分发，`_模板/` + `_分发/` + `.githooks/` + `scripts/` + 使用层方法文档 + `example_world/`）、**内容层**（本书创作产物，只留本地）、**开发层**（本文件 + `docs/development.md` + converge 治理记录，只留本地）。划分清单唯一权威源：`scripts/layers.py`。

分发只走 `python scripts/publish.py`（dry-run 预览 / `--force` 推送）：剥内容层与开发层 → 映射 `_分发/` → 零泄漏复核。`git push` 被 `.githooks/pre-push` 默认拒绝，推私有远端用 `--no-verify`。细节见 [[docs/development]]。
...
7. **检索排除**：全仓搜索时排除 `_模板/`（骨架镜像）与 `example_world/`（示例世界）——同名文件双份，结果翻倍
