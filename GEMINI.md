...
- **触发事件记载**：修改角色的核心欲望/核心恐惧/底线/应激模式 → 必须标注触发事件。
...
1. 确定在场角色列表
2. `python scripts/recall.py <角色1> <角色2>` — 获取 L2 注入集（agent 转写为自然语言，不粘贴 YAML）
3. `python scripts/relationship.py show <角色名>` — 两两关系摘要
4. 将以上与本场大纲、场景初始状态、**达成断言**（≥1 条主断言 + 覆盖在场角色的核心欲望/恐惧之一）组装为自然语言
