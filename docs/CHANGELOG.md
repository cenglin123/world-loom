...
### 卷级审计分维落地——审理素材「首轮全量、续轮增量」（B 计划执行）

#### 变更内容
- reviewer-protocol（含 _模板 成对）：「审理素材」节改为「首轮全量、续轮增量」并新增「续轮注入」小节——判据/基线/索引/上下文包每轮全量固定，正文为唯一增量（连续区间含邻章）；扩读受控（flags 声明→下轮并入）、风险判定对照 git diff、不确定恢复全量；执行前提=卷级场景清单存在
- reviewer-prompt-template（含 _模板 成对）：使用说明区补「注入清单以协议为单一源，模板不复制清单」；代码块不动，§③ 写后审不受影响
- docs/workflow.md §④「全量审计」→「首轮全量、续轮增量审计」
- 计划经 3 轮 converge 评议（硬阻断 4→3→0）后执行，计划文件移入 docs/plans/completed/
