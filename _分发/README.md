...
克隆或解压 release 之后，打开你的 agent（Claude Code / Cursor / 任何能读 `AGENTS.md` 的工具），说：
...
**已有旧版？** 见 [使用手册.md](使用手册.md) 的「更新到新版」——`python scripts/update.py` 只换工具不碰正文。
...
├── 06_文风样本/README.md                  # 文风模仿流程（init 生成；样本原文只留本地，不分发）
├── docs/
│   ├── workflow.md                      # 四阶段工作流细则
│   ├── writing-style.md                 # 叙述约定 + 可读性技法 + 故事驱动法则
│   ├── audit-checklist.md               # 一致性审计清单
│   ├── frontmatter-schemas.md           # 字段定义（唯一权威源）
│   ├── pitfalls.md                      # AI 写作已知陷阱
│   └── style-locked / decisions / CURRENT / CHANGELOG / plans   # init 生成的本书档案
...
| **文风契约** | 吸纳样本后 | 样本入库但规格未登记会被拦；调性漂移由审稿对照样本判 |
